...
import Modules.segmentierung as seg
import cv2
import os
...
for file_name in file_list:
    ...
    if file_name.endswith((".jpg", ".jpeg", ".JPG", ".png", ".bmp")):
        ...
        biggest_contour = bigcon.get_biggest_contour(
            con.get_contours(sigmentiertesImage)
        )
        ...
        biggest_align_contour = bigcon.get_biggest_contour(
            con.get_contours(sigmentiertesalignImage)
        )
        align_box, align_center, aligh_size, align_angle = bbox.boundary_box(
            biggest_align_contour
        )
