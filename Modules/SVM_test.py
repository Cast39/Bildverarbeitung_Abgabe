...
def run(image, result,settings=(5,50,20)):
  svm = cv2.ml.SVM_create()
  svm.setKernel(cv2.ml.SVM_LINEAR)
  #svm.setKernel(cv2.ml.SVM_RBF)
  svm.setType(cv2.ml.SVM_C_SVC)
  x = settings[2]
  svm.setC(x/10+0.1) # Größer -> weniger Klassen
  svm.setGamma(x/10.0+0.1) # Hohes Gamma -> Punkte nahe an Grenze haben höhere Priorität im Training
  size=40
  ...
  for i in range(sv.shape[0]):
      cv2.circle(lines, (int(sv[i,0]), int(sv[i,1])), 6, (128, 128, 128), 2)

  result.append({"name":"Input","data":input})
  result.append({"name":"Output","data":erg})
  result.append({"name":"Lines","data":lines})
